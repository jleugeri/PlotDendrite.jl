...
function _draw_branch(name, leaf_colors, relative_thickness, branch_color, sub_branches, args...; fill_connectors=false, connector_style= fill_connectors ? [] : [stroke("black"), linewidth(1)], style=[stroke("black")], h_frac=0.5, font_style=[stroke("black"), fill("white"), fontsize(12), linewidth(0.1)], kwargs...)
    branch_drawings = []
    branch_root_loc = Tuple{Float64,Float64}[]
    branch_total_lengths = Int64[]
    branch_total_branches = Int64[]
    for branch ∈ sub_branches
        branch_drawing, branch_total_length, branch_total_branches_, by0, by1 = _draw_branch(branch..., args...; fill_connectors=fill_connectors, connector_style=connector_style, style=style, h_frac=h_frac, font_style=font_style, kwargs...)
    ...
    branch_length = length(leaf_colors) + offset
    ...
    height  = (1/total_branches)*h_frac*relative_thickness#-1.5*1/(aspect)/total_length)*relative_thickness
    x0 = 1-width
    ...
    drawing = (
            ...
            (context(x0,0,width,1), fill(branch_color),
                ...
                if fill_connectors # filled connectors in parent context
                    ...
                    [(context(), [
                        curve((0,branch_ys[i]+0.5*(by0+by1)*branch_heights[i]), (0.7*offset/branch_length,branch_ys[i]+0.5*(by0+by1)*branch_heights[i]), (0.3*offset/branch_length,y0+height*i/(length(sub_branches)+1)), (offset/branch_length,y0+height*i/(length(sub_branches)+1))) for (i,(by0,by1)) ∈ enumerate(branch_root_loc)
                    ]..., connector_style...)]
                end...,
                
                if isempty(sub_branches)
                    # draw end-cap
            ...
            # draw synapses
            [
                (context(), arc(x0+(offset+i-1+0.5)*lw, y0, 0.4*lw, π, 2π), fill(lc)) for (i,lc) ∈ enumerate(leaf_colors)
            ]...,
