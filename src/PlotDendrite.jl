...
function plotDendrite(args...;canvas=nothing, caption="", caption_options=[stroke("white"), fill("black"), fontsize(8)], background="white", kwargs...)
    drawing, total_length, total_branches, _,_ = _draw_branch(args...; kwargs...)
    composed_drawing = compose(context(), (context(), drawing, (context(), text(1,1,caption,hright,vbottom), caption_options...), (context(), rectangle(), fill(background))))
    if canvas != nothing
